...
public class Disconnect extends BBDataBlock {
	public Disconnect(Player player) {
		...
		super(player.getName(), DISCONNECT, 0, player.getLocation().getBlockX(), player.getLocation().getBlockY(), player.getLocation().getBlockZ(), 0, "");
	}
	...
	private Disconnect(String player, int world, int x, int y, int z, int type, String data) {
		super(player, DISCONNECT, world, x, y, z, type, data);
	}
