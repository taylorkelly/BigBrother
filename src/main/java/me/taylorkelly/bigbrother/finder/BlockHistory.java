...
public class BlockHistory {
    ...
    public static ArrayList<BBDataBlock> hist(Block block, WorldManager manager) {
        ...
        try {
            ...
            ps = conn.prepareStatement("SELECT  bbdata.id, date, player, action, x, y, z, type, data, rbacked, bbworlds.name AS `world` FROM " + BBDataTable.getInstance().getRealTableName() + " INNER JOIN bbworlds ON bbworlds.id = bbdata.world  WHERE rbacked = 0 AND x = ? AND y = ?  AND z = ? AND bbdata.world = ? ORDER BY bbdata.id ASC;");
